...
gepu.monthly.pre <- fread("datasets/GEPU_Country_Data.csv") %>% filter(Year == "2019") %>% arrange(Month) %>% fill(everything(), .direction = "downup")
...
for (aspect in c("Prices", "Changes", "Returns", "Volatility")) {
  lower.aspect <- tolower(aspect)
  ...
  ts.data <- bind_cols(get(paste(lower.aspect, "monthly", sep = ".")), gpr.monthly.pre[, -1], gepu.monthly.pre[, -1])
  ts.data[-1] <- scale(ts.data[-1])
  melt.data <- reshape2::melt(ts.data, "Date", value.name = "Values", variable.name = "Index")
...
for (level in c("daily")) {
  for (aspect in c("Prices", "Changes", "Returns")) {
    ...
    melt.data <- get(paste(lower.aspect, level, "melt", sep = "."))
    ts.chart <- ggplot(data = melt.data, aes(x = Date, y = Values, color = Index)) +
      ggtitle(paste("Movement of", aspect, "with Economic Uncertainty Index (2020)")) +
      geom_line() + 
      center_title +
      guides(col = guide_legend(ncol = 20)) +
      theme(legend.position = "bottom", legend.direction = "vertical")
...
x.names <- names(prices.daily[-1])
x.names <- x.names[x.names != "IOST"]
cor.data <- left_join(prices.daily, gpr.daily.peri, by = c("Date" = "DATE"))
cor.data <- left_join(cor.data, teu.daily.peri, by = c("Date" = "Date")) %>% mutate(Date = as.Date(Date)) %>% select(!x.names)
ts.data <- cor.data
ts.data[-1] <- scale(ts.data[-1])
melt.data <- reshape2::melt(ts.data, "Date", value.name = "Values", variable.name = "Index")

ggplot(data = melt.data, aes(x = Date, y = Values, color = Index)) +
      ggtitle(paste("Prices with Policy Uncertainty Indices (2020)")) +
      geom_line() + 
      center_title +
      guides(col = guide_legend(ncol = 20)) +
      theme(legend.position = "none", legend.direction = "vertical") + facet_wrap(~Index)
...
for (level in c("daily")) {
  for (aspect in c("Prices", "Changes", "Returns")) {
    ...
    melt.data <- get(paste(lower.aspect, level, "melt", sep = "."))
    ts.chart <- ggplot(data = melt.data, aes(x = Date, y = Values, color = Index)) +
      ggtitle(paste("Movement of", aspect, "with Economic Uncertainty Index (2021)")) +
      geom_line() + 
      center_title +
      guides(col = guide_legend(ncol = 20)) +
      theme(legend.position = "bottom", legend.direction = "vertical")
