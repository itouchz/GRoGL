...
for (ticket in tickets) {
  ...
  ticket = gsub("\\.", "-", ticket)
  data <- read.csv(paste("datasets/daily/S&P_500/", ticket, '.csv', sep=""), header=TRUE)
  x = as.data.frame(data)
  x = x[cols]
  
  x$ticket = ticket
  x = subset(x, grepl("^2021", x$Date))
  sp500 <- rbind(sp500, x)
}


coinNames = list.files("datasets/daily/coins", pattern='*.csv', full.names=TRUE)

coins = data.frame(Date=character(), Close=numeric(), ticket=character())
i = 1
for (coin in coinNames) {
  data <- read.csv(coin, header=TRUE)
  x = as.data.frame(data)
  x = x[cols]
  y = strsplit(strsplit(coin, split="/")[[1]][4], split="-")[[1]][1]
  x$coin = y
  x = subset(x, grepl("^2021", x$Date))
  coins <- rbind(coins, x)
  coinNames[i] = y
...
effect = matrix(nrow=length(coinNames), ncol=length(tickets))
rownames(effect) <- coinNames
colnames(effect) <- tickets
effect
for (cn in coinNames) {
  for (t in tickets) {
      tm = gsub("\\.", "-", t)
      combined = merge(subset(coins, coins$coin == cn), subset(sp500, sp500$ticket == tm), by="Date")
      if (summary(aov(Close.x~Close.y, combined))[[1]][["Pr(>F)"]][1] < 2e-16) {
        effect[cn,t] = 1
      }
      else {
        effect[cn,t] = 0
...
total = matrix(0, nrow=1, ncol=length(coinNames))
colnames(total) <- coinNames
total
for (cn in coinNames) {
  for (t in tickets) {
    tm = gsub("\\.", "-", t)
    if (effect[cn,t] == 1) {
      total[1,cn] = total[1,cn] + 1
    }
  }
}
total

# write.csv(effect,"C:\\Users\\arnur\\CS564\\effect.csv", row.names=TRUE)
